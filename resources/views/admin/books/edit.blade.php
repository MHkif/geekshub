        ...
        <section class="w-full flex flex-col h-full gap-4 px-6">
            <div class="w-full flex-col items-center justify-between md:flex-row">
                <div class="w-full flex justify-between">
                    <h1 class="text-sm font-semibold leading-relaxed text-gray-600 md:text-lg">Books/Edit</h1>
                </div>
            ...
            <form method="POST" action="/admin/books/{{$book->id}}/update" enctype="multipart/form-data" class="mt-5">
                @csrf
                @method('PUT')
                <div>
                    <label for="title" class="block text-sm text-gray-700 capitalize dark:text-gray-200">Book Title</label>
                    <input placeholder="Al Moqadimuh" value="{{$book->title}}" name="title" type="text" class="block w-full px-3 py-2 mt-2 text-gray-600 placeholder-gray-400 bg-white border border-gray-200 rounded-md focus:border-indigo-400 focus:outline-none focus:ring focus:ring-indigo-300 focus:ring-opacity-40">
                    @error('title')
                ...
                <div class="mt-4">
                    ...
                    <input placeholder="Ibn Khaldoun"  value="{{$book->author}}" name="author" type="text" class="block w-full px-3 py-2 mt-2 text-gray-600 placeholder-gray-400 bg-white border border-gray-200 rounded-md focus:border-indigo-400 focus:outline-none focus:ring focus:ring-indigo-300 focus:ring-opacity-40" >
                    @error('author')
                ...
                <div class="mt-4">
                    <label for="category" class="block text-sm text-gray-700 capitalize dark:text-gray-200">Book's Category</label>
                     <select  value="{{$book->categoty_id}}" name="category_id" id="category" class="block w-full px-3 py-2 mt-2 text-gray-600 placeholder-gray-400 bg-white border border-gray-200 rounded-md focus:border-blue-400  focus:ring focus:ring-blue-300 focus:ring-opacity-40" aria-placeholder="category" aria-label="select a category" >
                ...
                <div class="mt-4">
                   ...
                   <textarea class="block w-full px-3 py-2 mt-2 text-gray-600 placeholder-gray-400 bg-white border border-gray-200 rounded-md focus:border-indigo-400 focus:outline-none focus:ring focus:ring-indigo-300 focus:ring-opacity-40"
                    ...
                    placeholder="Once Upon a time ...">{{$book->description}}</textarea>
